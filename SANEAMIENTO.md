...
| Repo | Rama | Commits |
|---|---|---|
| `backend` | `saneamiento/fase-1` | 6 |
| `frontend_react` | `saneamiento/fase-1` | 3 |
| `app_flutter` | `saneamiento/fase-2` | 1 |
| `iagentshub` | `saneamiento/fase-1` | 2 |

Ninguna rama está mergeada ni pusheada todavía.
...
| | Qué hacer |
|---|---|
| **FE-05** | `/openapi.json` **sigue expuesto en producción**: `app/api/app.py` pone `docs_url` tras `GAIA_DEV_MODE` pero no `openapi_url`. Cerrarlo, y de paso volcar el esquema en CI para generar los tipos de Dart y de la extensión. Hoy hay tres definiciones manuales del contrato y el backend puede romper cualquiera sin que falle una compilación: se mide en **380 `dynamic` en Dart frente a 0 `any` en TypeScript** |
| **FE-04** | `vs_code/src/url.ts` y `app_flutter/lib/core/config/backend_url.dart` validan la URL del backend con dos definiciones distintas de «local»: la extensión **rechaza un backend en `192.168.1.50` que Flutter acepta**. Portar la versión Dart, que es la completa, y dejar un test de tabla en ambos lenguajes |
| **BE-05** | `app/auth/auth.py` escribe la contraseña de admin **en claro** en `data/.admin_pass`. Mostrarla una vez al instalar y borrarla, o cifrarla con el Fernet que ya existe en `app/storage/crypto.py` |
| **BE-06** | El rate limit es por proceso y `main.py` arranca 4 workers: el límite real es ×4 y se pierde al reiniciar. Dividir entre `GAIA_WORKERS` y documentarlo; estado compartido en BD solo si el multiplicador resulta inaceptable en autenticación — medir antes de construir |
| **BE-09** | Chat, registro y los dos de billing parsean `request.json()` a mano; 12 de 24 routers ya usan pydantic con constraints, así que es copiar el patrón. `POST /api/billing/quote` no tiene ni auth ni rate limit mientras `/subscribe` sí |
